...
inline float4 intersect1d(float a_minimum,float a_maxinum, float b_minimum,float b_maximum)
{
    float4 i= __mm__cmple_ps(a_minimum,b_maximum);
    float4 j= __mm__cmple_ps(b_minimum,a_maximum);
    return __mm_and_ps(i,j);
}
...
int main()
{
    ...
    std::cout<<sum<<"\telapsed milliseconds: "<<std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count()<<endl;
    return 0;
